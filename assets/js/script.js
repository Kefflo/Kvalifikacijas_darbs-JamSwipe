...
// Funkcija atjaunina lietotāja vārdu.
function updateUsername(usernameClass) {
  // Iegūst ievadīto lietotājvārdu no attiecīgā ievades lauka.
  var usernameValue = $("." + usernameClass).val();

  // Veic AJAX POST pieprasījumu uz failu, lai atjauninātu lietotājvārdu.
  $.post("includes/handlers/ajax/updateUsername.php", {
    username: usernameValue, // Jaunais lietotājvārds.
    userLoggedIn: userLoggedIn, // Pašreizējais lietotājs.
  }).done(function (response) {
    // Atjaunina ziņojuma elementu ar servera atbildi.
    $("." + usernameClass)
      .nextAll(".message")
      .text(response);

    // Ja atbilde norāda uz veiksmīgu atjaunināšanu, atjauno redzamo lietotājvārdu.
    if (response.trim() === "Update successful") {
      $("." + usernameClass).val(usernameValue); // Atjauno ievades lauka vērtību.
      $("#navUsername").text(usernameValue); // Atjauno navigācijas joslas lietotājvārdu.
      userLoggedIn = usernameValue; // Atjauno globālo mainīgo ar jauno lietotājvārdu.
    }
  });
}
